...
# Tables and constants from the original implementation
...
TAU = [0x00, 0x08, 0x10, 0x18, 0x20, 0x28, 0x30, 0x38,
...
def transform_l(arr):
    # Convert to 8 64-bit integers (big-endian)
    u64 = [0]*8
    for i in range(8):
        u64[i] = int.from_bytes(arr[i*8:(i+1)*8], 'big')
    out = [0]*8
    for i in range(8):
        for j in range(64):
            if (u64[i] >> j) & 1:
    ...
    # Write back
    for i in range(8):
        arr[i*8:(i+1)*8] = array.array('B', out[i].to_bytes(8, 'big'))

def key_schedule(keys, idx):
    temp = keys[:]
    c_arr = array.array('B', C[idx])
    transform_x(temp, c_arr, keys)
    transform_p(keys)
    transform_l(keys)

def transform_e(keys, block, state):
    ...
    for i in range(12):
        ...
        key_schedule(keys, i)
        tmp = state[:]
        transform_x(tmp, keys, state)

def transform_g(n, h, m):
    keys = array.array('B', [0]*64)
    temp = array.array('B', [0]*64)
    transform_x(n, h, keys)
    # S-box omitted
    transform_p(keys)
    transform_l(keys)
    transform_e(keys, m, temp)
    tmp2 = temp[:]
    transform_x(tmp2, h, temp)
    transform_x(temp, m, h)

def add_512(a, b, res):
    a_int = int.from_bytes(bytes(a), 'big')
    b_int = int.from_bytes(bytes(b), 'big')
    s = (a_int + b_int) & ((1<<512)-1)
    res[:] = array.array('B', s.to_bytes(64, 'big'))

def single_block_hash(B_bytes, L):
    # B_bytes: 64-byte reversed padded block
    n = array.array('B', [0]*64)
    sigma = array.array('B', [0]*64)
    h = array.array('B', [0]*64)
    # block_size = 8*L (as 64-byte big-endian)
    block_size = array.array('B', [0]*64)
    chunk_size = L * 8
    block_size[62:] = array.array('B', chunk_size.to_bytes(2, 'big'))
    block = array.array('B', B_bytes)
    # Process block
    transform_g(n, h, block)
    add_512(n, block_size, n)
    add_512(sigma, block, sigma)
    # Final two calls
    zeros = array.array('B', [0]*64)
    transform_g(zeros, h, n)
    transform_g(zeros, h, sigma)
    # Reverse final hash
    h_rev = array.array('B', reversed(h))
    return bytes(h_rev)

# Target hash
target_hex = "5d71516b4a002b2c9065373f7ac7daa21f33826247eaa041ca5ef2e125d1e8dfea3198cc8b5b5821e33d8102cfae28eeda8919e6c5fd067f116cbf61532dee42"
target_bytes = bytes.fromhex(target_hex)

# Try all possible message lengths
for L in range(0, 65):
    print(f"Trying L = {L}")
    zero_hash = single_block_hash(bytes(64), L)
    y_int = int.from_bytes(target_bytes, 'big') ^ int.from_bytes(zero_hash, 'big')
    # Build matrix rows (512 rows, each a 512-bit integer)
    rows = [0] * 512
    for i in range(512):
        B_int = 1 << i
        B_bytes = B_int.to_bytes(64, 'big')
        h_i = single_block_hash(B_bytes, L)
        col = int.from_bytes(h_i, 'big') ^ int.from_bytes(zero_hash, 'big')
        for r in range(512):
            if (col >> r) & 1:
                rows[r] |= (1 << i)
    # Gaussian elimination over GF(2)
    aug = [(rows[r], (y_int >> r) & 1) for r in range(512)]
    rank = 0
    for col in range(511, -1, -1):
        pivot = None
        for r in range(rank, 512):
            if (aug[r][0] >> col) & 1:
                pivot = r
                break
        if pivot is None:
    ...
    for r in range(512):
        if aug[r][0] == 0:
            ...
            lead = aug[r][0].bit_length() - 1
            if aug[r][1]:
                x |= (1 << lead)
    if not ok:
        continue
    # Reconstruct B
    B_bytes = x.to_bytes(64, 'big')
    padded = B_bytes[::-1]  # reverse to get padded message
    if L < 64 and padded[L] == 1 and all(b == 0 for b in padded[L+1:]):
        message = padded[:L]
        print(f"Found flag: {message.decode()}")
        break
else:
    print("No flag found")