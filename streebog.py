...
def add_512(a: array.array, b: array.array, result: array.array):
    temp = 0
...
def transform_x(a: array.array, b: array.array, result: array.array):
    for i in range(64):
        result[i] = a[i] ^ b[i]
...
# подстановок PI.
def transform_s(result: array.array):
    for i in range(64):
...
def transform_p(result: array.array):
    temp = result[:]
    for i, position in enumerate(TAU):
        result[i] = temp[position]

# L-преобразование
# Представляет собой умножение 64-битного входного вектора на
# бинарную матрицу А размерами 64 на 64.
def transform_l(result: array.array):
    
    input_u64 = [0] * 8
    for i in range(8):
        input_u64[i] = int.from_bytes(result[i*8:(i+1)*8], 'big')
    ...
    for x in buffers:
        buffer.extend(x.to_bytes(8, 'big'))
...
def key_schedule(keys: array.array, iter_index: int):

    temp_keys = keys[:]
    transform_x(temp_keys, array.array('B', C[iter_index]), keys)
    transform_s(keys)
    transform_p(keys)
    transform_l(keys)

# E-преобразование
def transform_e(keys: array.array, block: array.array, state: array.array):
    ...
    for i in range(12):
        ...
        key_schedule(keys, i)
        temp_state = state[:]
        transform_x(temp_state, keys, state)


# Функция сжатия g
def transform_g(n: array.array, hash: array.array, message: array.array):
    ...
    temp_copy = temp[:]
    transform_x(temp_copy, hash, temp)
    transform_x(temp, message, hash)

def streebog_core(message: bytes, hash: array.array):
    ...
    for i in range(0, len(message), 64):
        ...
        if len(chunk) != 64:
            block = array.array('B', [0] * 64)
            block[:len(chunk)] = array.array('B', chunk)
            block[len(chunk)] = 1
        else:
            block = array.array('B', chunk)
        
        block.reverse()
        
        transform_g(n, hash, block)
        temp_n = n[:]
        add_512(temp_n, block_size, n)
        temp_sigma = sigma[:]
        add_512(temp_sigma, block, sigma)
    
    transform_g(array.array('B', [0] * 64), hash, n)
    transform_g(array.array('B', [0] * 64), hash, sigma)
    
    hash.reverse()

def streebog_512(message: bytes) -> array.array:
    hash = array.array('B', [0] * 64)
    streebog_core(message, hash)
    return hash

def streebog_256(message: bytes) -> array.array:
    hash = array.array('B', [1] * 64)
    streebog_core(message, hash)
    return hash[32:]
...
def main():
    ...
    hash512str = ''.join(f'{byte:02x}' for byte in hash512)
    
    hash256 = streebog_256(message)
    hash256str = ''.join(f'{byte:02x}' for byte in hash256)

    print(f"Streebog 512: {hash512str}")
    print(f"Streebog 256: {hash256str}")
